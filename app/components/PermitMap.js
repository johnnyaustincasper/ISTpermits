...
export default function PermitMap() {
  ...
  const changeStyle = useCallback((style) => {
    ...
    map.once('style.load', () => {
      ...
      map.on('click', 'permits-main', (e) => { const p = e.features[0].properties; setSelected({ ...p, production: p.production === true || p.production === 'true' }); });
      map.on('mouseenter', 'permits-main', () => { map.getCanvas().style.cursor = 'pointer'; });
      map.on('mouseleave', 'permits-main', () => { map.getCanvas().style.cursor = ''; });
    });
  }, [filtered]);
  ...
  return (
    <div style={{ width: '100%', height: '100vh', position: 'relative', overflow: 'hidden' }}>
      ...
      {/* Left Panel */}
      <div style={{ position: 'absolute', top: 64, left: 12, zIndex: 10, width: 230, display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 'calc(100vh - 80px)', overflowY: 'auto' }}>
        {/* Filters */}
        <div style={cardStyle}>
      ...
      {selected && (
        <div style={{
          ...
          minWidth: 420, maxWidth: 580, background: 'rgba(10,10,10,0.94)', border: '1px solid rgba(255,255,255,0.12)',
          borderRadius: 10, padding: '16px 20px', backdropFilter: 'blur(16px)',
